...
**训练：** `--data` (必需), `--epochs` (50), `--batch-size` (32), `--lr` (1e-4), `--resume`, `--force`

**推理：** `--model` (必需), `--pinyin` (必需), `--beam-size` (1), `--device` (auto)

**⚠️ 重要：** 拼音必须包含声调数字（1-4），如 `ni3 hao3` 而非 `ni hao`

**🔒 安全提示：** 训练前，系统会检查是否有未提交的代码更改。如果有未提交的更改，训练将暂停并提示您先提交。使用 `--force` 标志可以跳过此检查并继续训练。
...
| `--force` | flag | 无 | 忽略未提交的代码更改警告并继续训练 |
...
# 5. 有未提交更改时强制训练（跳过安全检查）
python model/train_pinhan.py \
    --data data/5k.jsonl \
    --epochs 40 \
    --save-dir outputs/experimental \
    --force
